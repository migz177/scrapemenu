...
# ─────────────────────────────────────────────
# AUTO-INSTALL & VERIFY PLAYWRIGHT BROWSER
# Tidak pakai @st.cache_resource karena Streamlit Cloud bisa
# recycle worker process tanpa full reboot — binary Chromium
# hilang tapi cache masih dianggap valid, sehingga install
# tidak diulang dan scraping gagal tanpa error yang jelas.
# Solusi: selalu verifikasi binary ada setiap app diload.
# ─────────────────────────────────────────────
def ensure_chromium_installed():
    """
    Verifikasi binary Chromium benar-benar ada di disk.
    Jika tidak ada, install ulang. Dipanggil saat app start
    dan sebelum tiap batch scraping untuk mencegah kegagalan
    akibat worker process recycle di Streamlit Cloud.
    """
    import subprocess, sys, os

    try:
        # Tanya Playwright di mana binary Chromium disimpan
        ...
        if exe_path and os.path.isfile(exe_path):
            return  # Binary sudah ada, tidak perlu install ulang
    except Exception:
        pass

    # Binary tidak ada atau tidak bisa dicek → install ulang
    subprocess.run(
...
def get_playwright_instance():
    """
    Satu Playwright instance yang di-cache sepanjang hidup server.
    
    MASALAH yang ini selesaikan:
    - Setiap klik tombol di Streamlit = script di-rerun dari atas
    - Membuka `with sync_playwright()` baru setiap run → resource leak
    - Setelah 1-2 run, memory habis, Chromium tidak bisa launch lagi
    - Di Streamlit Cloud memory sangat terbatas (~1GB)
    
    SOLUSI:
    - Instance dibuat SEKALI dan di-reuse semua run & semua user
    - Tidak ada overhead start/stop Playwright berulang kali
    - Kalau instance mati (crash), cache otomatis di-reset saat
      fungsi dipanggil lagi
    """
...
def kill_zombie_chromium():
    """
    Bunuh semua proses Chromium yang masih berjalan (zombie).
    Penting di Streamlit Cloud karena setiap scraping buka browser baru —
    jika tidak dibersihkan, proses menumpuk dan resources habis.
    """
    import subprocess
    try:
        subprocess.run(
...
LAUNCH_ARGS = [
    "--no-sandbox",
    "--disable-dev-shm-usage",          # Jangan pakai /dev/shm (terbatas di cloud)
    "--disable-gpu",
...
def main():
    ...
    # Bersihkan zombie dari run sebelumnya
    kill_zombie_chromium()

    # Ambil Playwright instance dari cache
    try:
        p = get_playwright_instance()
    except Exception:
        st.cache_resource.clear()
        ensure_chromium_installed()
        p = get_playwright_instance()

    # ── KUNCI: 1 browser untuk semua URL ──────────────────────────────────
    # Masalah sebelumnya: buka browser baru per URL → memory habis setelah
    # URL pertama karena browser lama belum benar-benar dibebaskan di cloud.
    # Solusi: 1 browser hidup sepanjang batch, tiap URL pakai context baru
    # (context = tab terisolasi, jauh lebih ringan dari full browser launch).
    # Setelah semua URL selesai, baru browser ditutup sekali.
    # ──────────────────────────────────────────────────────────────────────
    browser = None
    try:
        ...
        for idx, url in enumerate(raw_urls):
            ...
            try:
                # Tiap URL pakai context baru (terisolasi, cookies/cache terpisah)
                context = create_stealth_context(browser)
                page = context.new_page()

                detail_text.caption(f"  ↳ [{idx+1}/{total}] Membuka halaman GrabFood...")
                ...
                # Tutup hanya context (bukan browser) setelah tiap URL
                try:
                    if context:
                        context.close()
                except Exception:
        ...
        # Tutup browser SEKALI setelah semua URL selesai
        try:
            if browser:
                browser.close()
        except Exception:
